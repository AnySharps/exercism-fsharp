...
let move direction newNode zipper =
    newNode
    |> Option.bind (fun x ->
        Some
            { prev = Some zipper
...
let fromLeft left prev =
    { prev = prev.prev
      focus = { prev.focus with left = Some left }
      direction = prev.direction }

let fromRight right prev =
    { prev = prev.prev
      focus = { prev.focus with right = Some right }
      direction = prev.direction }
